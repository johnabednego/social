...
<?php
	require_once"database.php";
			$query=mysqli_query($link,"SELECT * from user where id='$id' order by id DESC") or die(mySQL_error());
...
<?php
	require_once"database.php";
			$query=mysqli_query($link,"SELECT * from photos where user_id='$id' ") or die(mySQL_error());
			while($row=mysqli_fetch_array($query)){
				$id = $row['photo_id'];
?>
