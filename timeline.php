...
<?php
	require_once"database.php";
							...
							if ($error > 0){
									...
									$update=mysqli_query($link," INSERT INTO post (user_id,post_image,content,created)
...
<?php
		require_once"database.php";
			$query= mysqli_query($link,"SELECT * from post LEFT JOIN user on user.id = post.user_id order by post_id DESC");
...
<?php
	require_once"database.php";
			$comment="SELECT * from comments where post_id='$post_id' order by post_id DESC";
			$stmt = mysqli_query($link, $comment);
			while($row=mysqli_fetch_array($stmt)){
...
?>
		 ...
		 <form  method="POST">			
			<div class="comment-area">
						...
						<?php 
						require_once"database.php";
						$image=mysqli_query($link, "SELECT * from user where id='$id'");
